...
TEST(AccessTest, GetKey_ValueNotPresent)
{
    HashTable* ht = createHashTable(hash, BUCKET_NUM);
    ...
    // Test if the hash table returns NULL when the value has been removed.
    EXPECT_EQ(NULL, getItem(ht, 0));
...
TEST(DeleteTest, RemoveNoValue)
{
    ...
    void *x = removeItem(ht, 1);
    
    // When the key is removed then insert should be able to put something at that key and return NULL
    EXPECT_EQ(NULL, removeItem(ht, 1));
...
TEST(DeleteTest, DeleteNoKey)
{
    ...
    // When the key is removed then insert should be able to put something at that key and return NULL
    EXPECT_EQ(NULL, insertItem(ht, 1, m[2]));
    
    destroyHashTable(ht);
}

TEST(DeleteTest, DeleteandInsert)
{
    HashTable* ht = createHashTable(hash, BUCKET_NUM);
...
TEST(DeleteTest, RemoveThenDelete)
{
    HashTable* ht = createHashTable(hash, BUCKET_NUM);
    ...
    //Shouldn't be any errors when you try to delete a nonexistent value
    EXPECT_EQ(NULL, insertItem(ht, 1, m[2]));
...
TEST(InsertTest, InsertWithnoValue)
{
    HashTable* ht = createHashTable(hash, BUCKET_NUM);
...
TEST(InsertTest, Insert_KeyNotPresent)
{
    HashTable* ht = createHashTable(hash, BUCKET_NUM);
